...
{
	[TestFixture]
	public class LiftUpdateProcessorRefactoredTests
	{
		...
		private class TestEnvironment : IDisposable
		{
			...
			public HgRepository GetProjAWebRepo(string projAWebWorkPath)
			{
			...
			public HgRepository CreateWebWorkRepository(string proja)
			{
		...
		/// </summary>
		[Test]
		public void ProcessLiftUpdates_OneProjectWithTwoUpdateFiles_CloneAndUpdatesProject()
		{
			const string rev0 = @"
<lift>
</lift>
";
			...
			{
				var repoA = env.CreateWebWorkRepository("ProjA");
				repoA.AddAndCheckinFile(rev0);

				var projAWebWorkPath = env.LangForgeDirFinder.CreateWebWorkProjectFolder("ProjA");
				//Make the webWork ProjA.LIFT file
				HgRepository projAWebRepo = env.GetProjAWebRepo(projAWebWorkPath);
				...
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate1, env.LangForgeDirFinder.LiftUpdatesPath);
				//Create another .lift.update file
				liftUpdateFileName = GetLiftUpdateFileName("ProjA", currentRevision, "extraB");
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate2, env.LangForgeDirFinder.LiftUpdatesPath);
		...
		public void Test_OneProject_MakeSureMergeWorkCopyIsNotOverWritten()
		{
			using (var env = new TestEnvironment())
			{
				...
				var projAMergeWorkPath = env.LangForgeDirFinder.CreateMergeWorkProjectFolder("ProjA");
				...
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate1,
												  env.LangForgeDirFinder.LiftUpdatesPath);
				...
				var xmlDoc = GetResult(env.LangForgeDirFinder.GetProjMergePath("ProjA"), "ProjA");
		...
		public void Test_OneProject_TwoUpdateFiles_VerifyUpdatesWereApplied()
		{
			using (var env = new TestEnvironment())
			{
				...
				//Create a .lift.update file. Make sure is has ProjA and the correct Sha(Hash) in the name.
				var liftUpdateFileName = GetLiftUpdateFileName("ProjA", currentRevision, "extraA");
				var liftUpdateFile1 = LiftUpdateFileFullPath(liftUpdateFileName, env);   //check this is deleted after updates are applied
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate1, env.LangForgeDirFinder.LiftUpdatesPath);
				//Create another .lift.update file
				liftUpdateFileName = GetLiftUpdateFileName("ProjA", currentRevision, "extraB");
				var liftUpdateFile2 = LiftUpdateFileFullPath(liftUpdateFileName, env); //check this is deleted after updates are applied
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate2, env.LangForgeDirFinder.LiftUpdatesPath);
				...
				//Check the contents of the .lift file
				AssertThatXmlIn.File(env.MergedLiftFile("ProjA")).HasAtLeastOneMatchForXpath("//entry[@id='one']/lexical-unit/form/text/value()=='ENTRY ONE ADDS lexical unit'");
		...
		public void Test_ProjA2Shas_Update1ToSha0ThenApplyUpdate2ToSha1()
		{
			using (var env = new TestEnvironment())
			{
				...
				XmlDocument xmlDoc;
				xmlDoc = GetResult(env.LangForgeDirFinder.GetProjMergePath("ProjA"), "ProjA");
...
<entry id='three' guid='80677C8E-9641-486e-ADA1-9D20ED2F5B69'></entry>";
		...
		public void Test_ProjA2Shas_ApplyUpdate2ToSha1ThenUpdate1ToSha0_ThenUpdate3ToSha2()
		{
			using (var env = new TestEnvironment())
			{
				...
				LfSynchronicMergerTests.WriteFile(liftUpdateFileName, s_LiftUpdate3, env.LangForgeDirFinder.LiftUpdatesPath);
		...
		private static string LiftUpdateFileFullPath(String filename, TestEnvironment env)
		{
			return Path.Combine(env.LangForgeDirFinder.LiftUpdatesPath, filename + SynchronicMerger.ExtensionOfIncrementalFiles);
		}
