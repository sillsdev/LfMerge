...
namespace LfMergeLift
{
	public class ProcessingState
	{
		...
		public static void Serialize(ProcessingState state)
		{
			...
			var fileName = LfDirectoriesAndFiles.Current.LfMergeStateFile(state.ProjectCode);
			File.WriteAllText(fileName, json);
		}

		public static ProcessingState Deserialize(string projectCode)
		{
			var fileName = LfDirectoriesAndFiles.Current.LfMergeStateFile(projectCode);
			if (File.Exists(fileName))
			{
				var json = File.ReadAllText(fileName);
				var serializer = new JavaScriptSerializer();
				return serializer.Deserialize<ProcessingState>(json);
			}
